...
const ackMessage = require('../lib/zwavemessages/ack.js');
...
suite('ACK', function() {
  test('Default constructor' +
    ...
    function(done) {
      var message = new ackMessage.getInstance();

      expect(message.mType).to.equal(null);
      expect(message.mFunctionID).to.equal(zutil.ACK);
  ...
  test('getData basic' +
       ...
       '\n\tExpectation: Association array should be returned with expected data for ACK',
    function(done) {
      var message = new ackMessage.getInstance();
      ...
      expect(messageData['functionID']).to.equal(zutil.ACK);
  ...
  test('generateMessageBuffer test' +
       '\n\tPurpose: Attempt to generate the message buffer for ACK' +
    ...
    function(done) {
      ...
      expect(messageRawBuffer[0]).to.equal(zutil.ACK);
